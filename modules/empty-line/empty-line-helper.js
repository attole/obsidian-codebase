class EmptyLineHelper {
  ...
  tryHandleLists(editor, prevLine, splitLine, newLine, nextLine, token) {
    ...
    if (!listItemRegex.test(fullLine)) {
      ...
      editor.setLine(newLine.position.line, "\n" + token);
      editor.setCursor({ line: nextLine.position.line + 1, ch: 0 });
      return true;
    }
    ...
    // end of list
    if (!nextLine || !listItemRegex.test(nextLine.content)) {
      editor.setLine(newLine.position.line, "\n" + token);
      editor.setCursor({ line: newLine.position.line + 2, ch: 0 });
      return true;
    }
