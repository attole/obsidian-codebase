...
class EmptyLineManager {
  ...
  // on editor change add observable there, not on startup
  /*run() {
		var targetNode = document.querySelector('div.markdown-reading-view');

		console.log(
			'NODE',
			targetNode,
			targetNode.dataset['emptyLine'],
			targetNode.style.display
		);

		if (targetNode.style.display !== 'none') {
			console.log('DISPLAY');
			//targetNode.dataset['emptyLine'] = 'true';
			el.querySelectorAll('span[data-token="emptyLine"]').forEach(
				(span) => {
					console.log('AAA');
					span.remove();
				}
		...
		if (targetNode.dataset['emptyLine']) return;

		var observer = new MutationObserver(function (mutations) {
			console.log('MUTATION START', mutations);
  ...
  async insertIntoEditor(editor, lineIndex) {
    ...
    handlers.find((fn) =>
      fn(editor, prevLine, splitLine, newLine, nextLine, this.EMPTY_LINE_TOKEN),
    );
